...
impl DeepFilterNetRealtime {
    #[new]
    #[pyo3(signature = (model_path=None, atten_lim=100.0, log_level=None, compensate_delay=true, post_filter_beta=0.0))]
    fn new(
        ...
        let _ = log_level;
        let model = create_model(model_path, atten_lim, post_filter_beta).map_err(to_py_err)?;
